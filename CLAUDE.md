...
2. **The `Plan` aggregate is the single authority** (`src/domain/aggregates/planner_orchestrator.py`):
   ...
   - 🔒 The domain is FROZEN (roadmap Phase 0). Additions require a deliberate, decision-logged un-freeze. So far: **#1** `AgentSpec.runtime_type/provider_id/model_id` (2026-07-05, agent registry owns runtime resolution); **#2** `Plan.planning_retry_not_before/planning_attempts` + the planning backoff gate (2026-07-08); **#3** the pause gate `Plan.paused/paused_reason` + `pause/resume/reopen_discovery`, `Task.retry/clear_backoff`, strict in-goal navigation, auto-pause replacing `fail_goal` (deleted), and nullable `AgentEvent.task_id` (2026-07-09).
...
- **AWAITING_REVIEW / REVIEW** — human gates; ALWAYS pause; unblocked by `approve` / `finish` / `replan`, plus `reopen_discovery` at AWAITING_REVIEW ("request changes" → DISCOVERY, the next commit REPLACES the un-executed roadmap).
- **RUNNING** — the pull-scan execution loop (`ExecutionHandler`): two-transaction writes, check-before-act idempotency, durable backoff gate (`retry_not_before`, **strict in-goal order** — a backing-off head task blocks its goal), tolerant finalize for late results after a replan. A terminal task failure **auto-pauses** the plan (recoverable) instead of failing it.
- **The pause gate** (un-freeze #3, not a phase): `Plan.paused` is a claim-predicate flag (`plans.paused`). A human `pause` or the auto-pause on exhaustion sets it; the worker stops claiming and goals/tasks become editable (`apply_edit` passes `plan.paused` to the edit-service guards; ops incl. `update_task`/`update_goal`/`remove_goal`). `resume` clears it and requeues FAILED tasks with a fresh budget (the manual retry) via `Task.retry()`. Terminal FAILED is reachable ONLY via `fail_plan` (permanent reasoner failure); `fail_goal` was deleted.
