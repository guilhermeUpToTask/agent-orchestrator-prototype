...
**The agent runner resolves via the AGENT REGISTRY + the providers catalog** (`praxis_orchestrator/infra/runtime/factory.py`) — the config key `agent_runner.mode` (`dry-run` default | `real`, plus `agent_runner.timeout_seconds` 600) picks the global mode; `dry-run` is the `DummyAgentRunner` and never touches the secret store. In `real` mode the `CatalogAgentRunner` resolves **per task, per run** from the bound `AgentSpec`: `runtime_type` (`pi` default | `claude` | `codex` | `gemini` | `dry-run`) picks the CLI runtime, `provider_id`/`model_id` rows supply the envelope-encrypted key and model string (pi's backend derives from the provider id/name against `PI_BACKEND_ENV_VAR`). A broken binding raises `TaskFailed(AUTH_ERROR)` (terminal); write-time referential checks + `AGENT_RUNNER_CONFIG_INVALID` → 422; `GET /api/runner/status` reports mode/bindings/binary probes (`dependency_checker.py`) and the worker warns at boot in real mode. Providers/models bound to an agent are delete-guarded (409).

**The cycle acceptance run resolves from config too** (`ProjectEnvironment`, `praxis_orchestrator/app/environment_port.py` — a port the FROZEN domain never sees). Orchestrator-scoped keys: `environment.mode` (`container` selects `ContainerEnvironment`; **anything else, including a typo, falls back to `NoEnvironment`** — the verdict is advisory and must never take down the gate it observes) and `environment.container_binary` (`docker` default | `podman` | any CLI-compatible runtime — which one exists is a property of the MACHINE, hence orchestrator scope). Project-scoped keys author the boot itself and are the OPERATOR's, never a model's: `environment.image | command | port | healthcheck | scenario | startup_timeout_seconds` (`infra/environment/spec.py`). `NoEnvironment` is the PERMANENT fallback like `NoSandbox`/`NoForge` and records `skipped`, which `is_signal` distinguishes from a pass. `verify()` MUST NOT raise. `startup_timeout_seconds` budgets the APPLICATION becoming healthy, never the daemon call — conflating them leaked containers (P8.5).

### Frontend (TypeScript / React / Vite)
- **Install**: `npm install`
- **Run Dev**: `npm run dev`
- **Build**: `npm run build` (tsc + vite)
- **Stage the UI into the wheel**: `backend/scripts/build_frontend.sh` — builds `frontend/dist` into `backend/praxis_orchestrator/api/static/` (git-ignored) so `uv build` ships one artifact. The release workflow runs it BEFORE `uv build` and then verifies the wheel carries both the UI and the migrations.
- **Browser checks**: two suites, two questions, two configs — merging them would make one of them lie about the state it runs against.
  - `npm run test:e2e` — the SMOKE suite (`playwright.config.ts`): does the packaged bundle BOOT, and what does a FRESH install show? Runs `--no-worker` against a wiped state directory.
  - `npm run test:e2e:cycle` — the FULL-CYCLE suite (`playwright.cycle.config.ts`, `e2e/cycle/`): can an operator drive a brief to a published cycle in the browser? Seeded catalog + live worker, **Tier 0** (stub + dry-run), whole cycle in seconds. Added 2026-08-10 as Phase 9's first act — until then **no test had ever driven the console through a plan**, and this file claimed that was "Phase 8" from Phase 6 onward while Phase 8 came and went without it.
  - Both run in CI. The cycle suite is the safety net the Phase 9 refactor rests on, so it addresses controls by **role and accessible name only** — never CSS classes, never test ids. A control it cannot address by name is a control a screen reader cannot announce.
- **Driving the browser by hand** (agent-friendly): `npx playwright cli` — open/goto/click/fill/snapshot/find/eval/requests/screenshot, built into `playwright-core`. **Not the MCP server**: `playwright mcp` and `@playwright/mcp` both exist and were rejected 2026-08-10 as expensive per interaction and slower than a shell command that prints a snapshot and exits. It defaults to branded Chrome, so `.playwright/cli.config.json` pins it to the bundled chromium.
...
1. **Dependency Rule**: `domain` -> `app` -> `infra` & `api`.
   - **NEVER** import `app`, `infra`, or `api` modules inside `praxis_orchestrator/domain/`.
   - **NEVER** import `infra` inside `praxis_orchestrator/app/` (App uses Ports; Infra provides Adapters). The in-memory fakes live in `praxis_orchestrator/app/testing/fakes.py`; infra re-exports what it shares (e.g. the dummy runner).
2. **Plan disposal is a repository command, not a transition**: the cyclic root
   ...
   it does not.
3. **The `Plan` aggregate is the single authority** (`praxis_orchestrator/domain/aggregates/planner_orchestrator.py`):
   ...
   - 🔒 The domain is FROZEN (roadmap Phase 0). Additions require a deliberate, decision-logged un-freeze — the full, authoritative list lives in [`docs/decisions/decision-log.md`](docs/decisions/decision-log.md) and currently runs through **un-freeze #20** (decision 64, 2026-08-10: agents can be rebound on a PAUSED plan — `rebind_agents` re-points a goal's unfinished tasks at different agents, so moving in-flight work to another runtime no longer means deleting the plan and losing its intent, contracts and evidence). The pivotal one is **#4 — [ADR-003](docs/decisions/adr-003-cyclic-project-plan-lifecycle.md) (decision 43, 2026-07-14): the cyclic `ProjectPlan` lifecycle**, which superseded the terminal nine-phase machine (see Key Concepts below). Earlier un-freezes: **#1** agent-registry runtime resolution (2026-07-05); **#2** the planning backoff gate (2026-07-08); **#3** the pause gate + recoverable auto-pause + editable-while-paused (2026-07-09). Do not re-summarize the whole list here — link the decision log.
...
7. **Configuration & DI**: the environment is read ONLY in `AppContainer` (`praxis_orchestrator/infra/container.py`, the composition root) — never deep in the code. All adapters hang off the container as cached properties; `new_unit_of_work()` per worker/request (a UoW is not thread-safe).
8. **Error Handling**: domain errors subclass `DomainError` with a stable `code`; the CLI uses `@catch_domain_errors`; the API maps codes to HTTP statuses in ONE table (`praxis_orchestrator/api/exceptions.py::_STATUS_BY_CODE`). Do NOT scatter try/except returning HTTP responses inside routers, and do NOT add blanket `KeyError`/`ValueError` handlers.
...
- The repository's detected default branch is never touched by plan work. One output disposition (`open_pr | merge | retain_branch | discard`) is recorded per cycle. **`open_pr` really opens one** (P8.1, decision 61): `app/forge_port.py` is the port (beside `sandbox_port.py`, never seen by the domain), `infra/forge/github.py` the GitHub-only adapter, `NoForge` the permanent fallback. The forge binding lives in the PROJECT-SCOPED CONFIG STORE (`forge.provider|repository|token_ref`) and the token in the secret store at `secret://forge/<project_id>` — so `ProjectDefinition` is untouched and no un-freeze was needed. `app/use_cases/publish_cycle.py` pushes and calls the API OUTSIDE any transaction, then records the disposition with the real PR URL; a forge failure leaves the gate open with nothing written. The port has NO merge method: the orchestrator opens a PR, never merges one, and pushes only `cycle/<id>`.
...
- Frontend routes: `/` (plan list + composer), `/plans/:id` (Overview / Goals canvas / Agents / Activity, with the chat panel, the intent/draft/publication review-gate dialogs, and block-resolution controls), `/settings/*`, and `/docs/:slug` (the in-console manual — it renders the repository's OWN `docs/guides/*.md` via `import.meta.glob`, so there is exactly one copy of every guide; never hand-write a second version for the browser). SSE events are NAMED (`event: <type>`), so the client registers per-type listeners. The status/gate/block surface is driven by backend `status`/`activity`/`legal_actions`; some secondary panels still render legacy phase history (see [`docs/architecture/frontend.md`](docs/architecture/frontend.md)).
...
2. **Unit tests**: domain invariants + use cases against `praxis_orchestrator/app/testing/fakes.py` (`InMemoryPlanRepository`, `DummyAgentRunner` scripted per task id, `FakeClock.advance()` for backoff/lease determinism).
...
## 🧹 Code Style & Types
- **Python**: `mypy praxis_orchestrator` must pass with zero errors and NO exclude list; `praxis_orchestrator/domain` and `praxis_orchestrator/app` are fully strict, `praxis_orchestrator/infra`/`praxis_orchestrator/api` carry the documented relaxations in `pyproject.toml` — tighten over time, never loosen. Use `from __future__ import annotations`. Pydantic `BaseModel` for DTOs/VOs/entities; `@dataclass(frozen=True)` for immutable structures where Pydantic isn't needed.
...
praxis-orchestrator/
├── backend/
│   ├── praxis_orchestrator/   # the installed package (NOT `src/` — imports are
│   │   │                     #   `praxis_orchestrator.…`, and every path below is
...
│   │                       # praxis_orchestrator/infra/db/migrations/versions/
