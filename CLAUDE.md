...
- **DB migrations**: `python -m agent_orchestrator.infra.cli.main db upgrade` (DB lives under `ORCHESTRATOR_HOME`, default `~/.orchestrator`)
- **Run everything (one command)**: `python -m agent_orchestrator.infra.cli.main serve --port 8000` — migrates, starts the API with the packaged UI, and supervises a worker subprocess (`--no-worker` / `--no-migrate` to opt out). This is the installed-user path; the two commands below stay for development.
...
Environment: `ORCHESTRATOR_HOME` (state dir), `ORCHESTRATOR_MASTER_KEY` (Fernet key for the secret store), `ORCHESTRATOR_API_TOKEN` (auth for EVERY operation except `GET /health`; open when unset; `GET /api/events` also accepts it as `?token=` because EventSource cannot send headers). There is NO `AGENT_MODE` env var — runtime selection lives in SQLite, and **`PROJECT_REPO_DIR` is not read by anything** (locked by `tests/unit/test_fixture_docs_contract.py`): a plan reaches a repository through its `ProjectDefinition.repo_url`, validated on write by `agent_orchestrator/infra/git/repository_binding.py`, and a project without one gets a scratch repo it is told about.
...
1. **Dependency Rule**: `domain` -> `app` -> `infra` & `api`.
   - **NEVER** import `app`, `infra`, or `api` modules inside `agent_orchestrator/domain/`.
   - **NEVER** import `infra` inside `agent_orchestrator/app/` (App uses Ports; Infra provides Adapters). The in-memory fakes live in `agent_orchestrator/app/testing/fakes.py`; infra re-exports what it shares (e.g. the dummy runner).
2. **Plan disposal is a repository command, not a transition**: the cyclic root
   ...
   it does not.
3. **The `Plan` aggregate is the single authority** (`agent_orchestrator/domain/aggregates/planner_orchestrator.py`):
...
7. **Configuration & DI**: the environment is read ONLY in `AppContainer` (`agent_orchestrator/infra/container.py`, the composition root) — never deep in the code. All adapters hang off the container as cached properties; `new_unit_of_work()` per worker/request (a UoW is not thread-safe).
8. **Error Handling**: domain errors subclass `DomainError` with a stable `code`; the CLI uses `@catch_domain_errors`; the API maps codes to HTTP statuses in ONE table (`agent_orchestrator/api/exceptions.py::_STATUS_BY_CODE`). Do NOT scatter try/except returning HTTP responses inside routers, and do NOT add blanket `KeyError`/`ValueError` handlers.
...
2. **Unit tests**: domain invariants + use cases against `agent_orchestrator/app/testing/fakes.py` (`InMemoryPlanRepository`, `DummyAgentRunner` scripted per task id, `FakeClock.advance()` for backoff/lease determinism).
...
agent-orchestrator/
├── backend/
│   ├── agent_orchestrator/   # the installed package (NOT `src/` — imports are
│   │   │                     #   `agent_orchestrator.…`, and every path below is
