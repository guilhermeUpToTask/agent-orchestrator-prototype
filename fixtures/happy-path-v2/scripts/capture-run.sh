#!/usr/bin/env bash
# Collect one happy-path-v2 run into a single consistently named directory.
#
...
#
# Writes to  $ORCHESTRATOR_HOME/happy-path-v2/runs/<UTC>-tier<N>-<plan prefix>/
#
...
SEED_SHA="$(git -C "$HAPPY_PATH_REPO" rev-parse "happy-path-v2-seed" 2>/dev/null || echo unknown)"
