#!/usr/bin/env bash
# Create (or re-seed) the disposable happy-path-v2 git repo outside the monorepo.
set -Eeuo pipefail
IFS=$'\n\t'
...
if [[ ! -d "$TARGET/.git" ]]; then
  ...
  git -C "$TARGET" config user.email "happy-path-v2@localhost"
  git -C "$TARGET" config user.name "happy-path-v2"
else
...
if git -C "$TARGET" diff --cached --quiet; then
  ...
  git -C "$TARGET" commit -m "happy-path-v2 seed: greet NotImplemented + failing test"
fi
...
note "then: POST fixtures/happy-path-v2/brief.txt as a plan -- see the fixture README"
