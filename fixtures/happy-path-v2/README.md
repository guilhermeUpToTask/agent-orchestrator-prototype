...
├── acceptance/               ← the fixture's OWN verdict, never enters the repo
├── seed/                     ← file templates (not a git repo)
│   ├── README.md
│   ├── pyproject.toml
│   ├── src/happy_path/...
│   └── tests/                 ← EMPTY: the agent authors the check here
└── scripts/
    ...
    ├── check-success.sh      ← run acceptance/ against a checkout (expectation 7)
...
### Why the acceptance check lives outside the repo

v1 ran `pytest` **inside** the repo — the same `tests/` the agent writes to — so
the fixture's verdict was not independent of the agent's work: a weak test the
agent wrote and then implemented to would pass. v2 keeps `acceptance/` in the
fixture directory; `check-success.sh` copies it into a throwaway checkout, runs
it, and deletes it. It asserts three things, and the third is the one v1 could
never make:

1. `greet("Ada") == "Hello, Ada!"`
2. the agent authored a check that mentions `greet`
3. **that check discriminates** — `greet` is replaced with `return ""` in a
   scratch copy and the agent's own tests must FAIL. A test like
   `def test_greet(): greet("Ada")` satisfies (1) and (2) while asserting
   nothing; only the mutation probe catches it.
