...
def project_status():
    ...
    project = ctx.machine.project_name
    if not project:
    ...
    click.echo(f"  Redis URL      : {ctx.machine.redis_url}")

    project_home = ctx.machine.orchestrator_home / "projects" / project
...
def project_list():
    ...
    ctx = SettingsService().load()
    active = ctx.machine.project_name
    projects_root = ctx.machine.orchestrator_home / "projects"
...
def project_use(name: str):
    ...
    ctx = SettingsService().load()
    project_home = ctx.machine.orchestrator_home / "projects" / name
...
def project_reset(yes: bool, keep_agents: bool):
    ...
    if not yes:
        ...
        click.confirm(
            f"  Reset project '{ctx.machine.project_name}' (tasks{extra} + git branches)?",
            abort=True,
        )

    result = build_project_reset_usecase(ctx).execute(keep_agents=keep_agents)
    ...
    if result.had_errors:
        ...
        ok(f"Project '{ctx.machine.project_name}' reset complete")
