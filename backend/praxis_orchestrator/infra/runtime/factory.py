"""
praxis_orchestrator/infra/runtime/factory.py — build the AgentRunner from the agent registry
and the providers catalog (mirrors praxis_orchestrator/infra/reasoner/factory.py).
...
from praxis_orchestrator.app.observations import ObservationRepository
...
from praxis_orchestrator.infra.db.secret_store import SqliteSecretStore
...
from praxis_orchestrator.infra.runtime.registry import (
    RUNTIME_REGISTRY,
    RuntimeBuild,
    runtime_names,
)
from praxis_orchestrator.infra.runtime.dummy_runner import DryRunAgentRunner
...
def validate_agent_runner_mode(config_store: SqliteConfigStore) -> RunnerModeStatus:
    ...
    return RunnerModeStatus(
        ...
        detail=(
            ...
            "'real'. Fix it with: "
            "praxis config set agent_runner.mode dry-run|real"
        ),
    )
...
class CatalogAgentRunner:
    ...
    def _runner_for(self, spec: AgentSpec) -> AgentRunner:
        ...
        if descriptor.build is None:
            ...
            # store, so dry-run keeps working without PRAXIS_MASTER_KEY.
            return self._dummy
...
def build_agent_runner(
    ...
    `sandbox` (ROADMAP item 33) defaults to NoSandbox — today's behavior —
    when not supplied; dry-run never touches a real subprocess so it never
    needs one."""
