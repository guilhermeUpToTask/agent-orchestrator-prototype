"""
praxis_orchestrator/infra/cli/main.py — the praxis CLI (fundamental commands only,
roadmap 4.2).

    praxis version [--json]            what this install is, for a report
    praxis db upgrade                  run migrations to head
    praxis api start [--port]          serve the FastAPI app
    praxis worker start [--worker-id]  run the claim-and-drive worker
    praxis config get|set|list         two-tier config (SQLite)
    praxis plan list|show              read-only plan inspection
...
def _package_version() -> str:
    ...
    try:
        return version("praxis-orchestrator")
...
def _commit() -> str:
    ...
    the working directory: `praxis version` run from inside some other
...
def version_cmd(as_json: bool) -> None:
    ...
    if as_json:
        ...
        return
    click.echo(f"praxis-orchestrator {facts['version']}")
...
@catch_domain_errors
def db_upgrade() -> None:
    """Apply migrations up to head for the PRAXIS_HOME database."""
    from alembic import command

    from praxis_orchestrator.infra.container import AppContainer
    from praxis_orchestrator.infra.db.engine import db_url_for_home
    from praxis_orchestrator.infra.db.migration_config import alembic_config
...
def api_start(host: str, port: int) -> None:
    ...
    # headers. See praxis_orchestrator/api/security.py::require_api_token_or_query.
...
def seed_demo(
    ...
    Stub mode:  praxis seed demo --stub
    LLM mode:   praxis seed demo --provider openrouter --model <name> \\
    ...
    from praxis_orchestrator.domain.errors.config_errors import EntityAlreadyExistsError
    ...
    from praxis_orchestrator.infra.runtime.cli_runner import PI_BACKEND_ENV_VAR
