...
2. **Adapter semantics must match the in-memory fakes** (`praxis_orchestrator/app/testing/fakes.py`) — detached aggregates, the CAS shape, lease expiry. The dual-backend truth suite (`tests/support.py`) enforces this; when you change an adapter contract, change the fake in the same PR.
...
│                       The ONLY reader of PRAXIS_HOME / PROJECT_REPO_DIR / etc.
...
│   │                   fails closed on a missing PRAXIS_MASTER_KEY.
