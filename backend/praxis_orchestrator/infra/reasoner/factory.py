"""
praxis_orchestrator/infra/reasoner/factory.py — build the Reasoner from the providers catalog.
...
Two modes, selected by the config key `reasoner.mode` (scope 'orchestrator'):
  ...
  stub (default) — the deterministic StubReasoner. NEVER touches the secret
                   store, so dry-run works without PRAXIS_MASTER_KEY.
...
from praxis_orchestrator.app.ports import PriorPlanningAttempts, RepositoryReader
...
from praxis_orchestrator.domain.repositories.capability_repo import CapabilityRepository
    ...
    if mode != "llm":
        return ReasonerConfigStatus(
            ...
            detail=(
                ...
                "Fix it with: praxis config set reasoner.mode stub|llm"
            ),
        )
    ...
    if not provider_id:
        return ReasonerConfigStatus(
            ...
            detail=(
                ...
                "Seed one with `praxis seed demo --provider ...` or set it "
                "with `praxis config set reasoner.provider_id <id>`."
            ),
        )
    ...
    if not model_id:
        return ReasonerConfigStatus(
            ...
            detail=(
                ...
                "Set it with `praxis config set reasoner.model_id <id>`."
            ),
        )
