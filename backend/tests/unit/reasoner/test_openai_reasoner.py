...
from praxis_orchestrator.domain.ports.reasoner_port import ChatMessage
...
def test_converse_records_reported_usage_with_provenance():
    from praxis_orchestrator.app.observations import ObservationQuality, ObservationSource
...
def test_enrich_records_missing_usage_as_unavailable_not_zero():
    from praxis_orchestrator.app.observations import ObservationQuality
    from praxis_orchestrator.app.testing.observations import InMemoryObservationRepository
...
def test_enrichment_schema_states_what_allowed_scope_means():
    ...
    from praxis_orchestrator.infra.reasoner.openai_reasoner import SUBMIT_GOAL_CONTRACT_SCHEMA
...
class FakeRepositoryReader:
    ...
    def orientation(self, project_id):
        self._guard()
        from praxis_orchestrator.app.ports import RepositoryOrientation
...
def _fingerprint_for(plan, goal):
    from praxis_orchestrator.infra.reasoner.openai_reasoner import _enrichment_fingerprint
...
def test_the_escalating_budget_is_capped():
    ...
    from praxis_orchestrator.infra.reasoner.openai_reasoner import MAX_ENRICH_TURN_ESCALATION
