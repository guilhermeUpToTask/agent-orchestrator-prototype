...
(`praxis_orchestrator/api/security.py`). uvicorn's access logger would then print
...
def test_api_start_disables_the_uvicorn_access_log(monkeypatch):
    ...
    monkeypatch.setattr("praxis_orchestrator.api.server.create_app", lambda: object())
