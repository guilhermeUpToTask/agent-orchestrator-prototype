...
`acceptance_runs` records one row per **advisory** cycle acceptance run (migration `0018_acceptance_runs`, P8.2). A `ProjectEnvironment` adapter boots the assembled cycle branch and runs the operator's scenario against it, at each goal merge and again before the publication gate; the row holds the trigger, the ref booted, the outcome (`passed | failed | errored`), a summary and detail, and the duration. It is deliberately an operational ledger rather than plan state — the verdict gates nothing, which is why the acceptance run needed no domain un-freeze — and `plan_id` uses `ON DELETE CASCADE`. A `skipped` verdict is **not** recorded: no environment configured is the default state, and one row per goal merge saying "nobody asked" would be noise. That is why an empty list means nobody asked rather than everything passed.
