...
Code anchors: `backend/praxis_orchestrator/app/use_cases/run_worker.py` (the loop), `backend/praxis_orchestrator/infra/db/plan_repository.py` (the lease SQL), `backend/praxis_orchestrator/app/execution_records.py` + `backend/praxis_orchestrator/infra/db/execution_record_repository.py` (stable run/attempt identity), `backend/praxis_orchestrator/app/handlers/execution_handler.py` (the two-transaction choreography) with its collaborators `backend/praxis_orchestrator/app/handlers/agent_admission.py` (which agent runs, whether it may start, and the `RuntimeCircuit` rows on both sides) and `backend/praxis_orchestrator/app/handlers/goal_promotion.py` (reserve, merge, observe), `backend/praxis_orchestrator/domain/services/navigation.py` (the scan), `backend/praxis_orchestrator/infra/git/workspace.py` (the rollback), `backend/praxis_orchestrator/infra/runtime/` (the runners).
...
- **Crash recovery separates authority from evidence.** Lease expiry lets another worker resume persisted domain state. Worker startup marks orphaned operational attempts/runs ABANDONED only when NEITHER the plan claim NOR the owning goal's lease is live; it leaves the task RUNNING for the existing reclaim choreography. Checking the plan claim alone was a real defect: since per-goal leases, attempts are created by goal workers, which do not hold the plan claim, so a second worker's startup abandoned the ledger row of an attempt that was alive and about to finalize (locked by `test_startup_reconciliation_respects_a_live_GOAL_lease`). Startup also runs conservative `git worktree prune` and audits worktrees/refs. Lease times use the injected `Clock`, so fake and SQLite truth tests drive expiry deterministically.

Concurrency is **per goal, sequential within one**. [ADR-001](../decisions/adr-001-concurrency-lease.md) made the lease granularity (plan → goal → task) the parallelism switch, and domain un-freeze #14 turned the goal rung on: independent ready goals are claimed on their own `goal_leases` rows and driven concurrently, while the tasks inside one goal stay a strict chain. A single `praxis worker start` owns an in-process pool of goal drivers (`--max-concurrent-goals`, default 4), each with its own `UnitOfWork`; a second worker process is for horizontal scale, not for parallelism within a plan. The plan-level claim remains and is what serialises planning, gates and enrichment for one plan.
...
canonical form, inject only `PRAXIS_PLAN_ID`, `PRAXIS_GOAL_ID`,
`PRAXIS_TASK_ID`, `PRAXIS_RUN_ID`, `PRAXIS_ATTEMPT_NUMBER`,
and `PRAXIS_ATTEMPT_ID` into the child environment. The task prompt is a
...
- `begin` → on a cyclic plan, branch `task/<task_id>/<run_id>` off `goal/<goal_id>` (itself cut off `cycle/<cycle_id>`, which is cut off the repository's detected default branch); on a legacy plan, branch `task/<task_id>/a<attempt-number>` off `plan/<plan_id>`. Plus a temp worktree. The cyclic form keys on the globally unique run id so a retry never reuses a prior attempt's branch; the legacy attempt number is monotonic across the task lifetime, unlike the resettable domain retry counter. `branch -f` makes begin idempotent against a crashed prior invocation of that number. Names come from `praxis_orchestrator/app/branch_names.py`.
