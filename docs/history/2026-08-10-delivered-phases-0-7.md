# Delivered phases 0–7 — the full record

*Archived 2026-08-10 (P8.7 task 6).* This is the delivery narrative for Phases
0 through 7, moved verbatim out of [`ROADMAP.md`](../../ROADMAP.md).

Nothing here was deleted or rewritten; it was in the wrong file. The roadmap's
own stated job is "everything planned but not yet implemented (+ do-not-do)",
and by the time Phase 8 opened it carried 1,096 lines of what had ALREADY been
done — measurements, defect post-mortems, exit-criteria evidence and dated
delivery status for eight closed phases. That is worth keeping and worth being
able to read; it is not a plan.

The roadmap now summarises each closed phase in a row and links here. Current
behaviour is described by [`docs/architecture/`](../architecture/); decisions by
[the decision log](../decisions/decision-log.md); verified open defects by
[known-issues.md](../architecture/known-issues.md). This file is the paper
trail behind them.
...
- **A task's checks are identified by declaration** (`praxis_orchestrator/app/test_identity.py`),
...
Fixed by a per-scope curve in `praxis_orchestrator/app/provider_capacity.py`
...
- **P4.3 — evidence truth** (G9): ✅ complete; the
  ...
  - Plan: `docs/history/planning/2026-07-28-phase-4-3-evidence-truth.md`
    (9 tasks; execution ledger at
    `.superpowers/sdd/2026-07-28-phase-4-3-evidence-truth/progress.md`)
  - **Complete:** branch-naming module (`praxis_orchestrator/app/branch_names.py`), migration
...
- ✅ **Migrations ship inside the package.** `db_upgrade` resolved them from
  ...
  could not create its schema at all. Moved to `praxis_orchestrator/infra/db/migrations/`, one
  resolver (`migration_config.py`) derived from its own module location.
- ✅ **The UI ships inside the wheel** and is served by the API
  (`praxis_orchestrator/api/frontend.py`), so there is no second artifact and no CORS story on
...
`src/…` → `praxis_orchestrator/…` rename changed the published contract. All
three fixed on the branch. The browser smoke passed on its first CI run.

- ✅ **The distribution package is `praxis_orchestrator`.** Renamed from a
...
- `backend/praxis_orchestrator/api/static/` is generated by
