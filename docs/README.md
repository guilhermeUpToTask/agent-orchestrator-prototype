...
The backend's **frozen port contracts** (exact SQL shapes, method signatures, API→use-case map) live next to the code: [`backend/docs/INTEGRATION_GUIDE.md`](../backend/docs/INTEGRATION_GUIDE.md). Per-layer and per-package READMEs live inside `backend/praxis_orchestrator/` — they are the closest documentation to each line of code.
...
- `architecture/` files cite real paths (`backend/praxis_orchestrator/...`); when you rename code, grep the docs.
